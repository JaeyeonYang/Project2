...
주의사항:
-각 카테고리당 최대 8개까지만 포함
-너무 일반적이거나 모호한 단어(예: "computer", "software", "programming")는 제외
-키워드는 CV에서 실제로 언급된 내용만 추출
-영어와 한글 키워드를 모두 허용하되, 대소문자와 표기법은 CV 원문을 최대한 반영
-신뢰도(confidence)는 high/medium/low 중 하나로 판단
-반드시 위 JSON 예시와 동일한 구조와 순서로만 응답(불필요한 텍스트, 설명, 주석, 마크다운 없이 JSON만 반환)
-각 키워드는 중복 없이 한 번만 포함
"""
